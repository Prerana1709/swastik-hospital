// Role & Permissions: create role, toggle module access. Local state only.
import React, { useState } from "react";
import { Link } from "react-router-dom";
import { FaArrowLeft } from "react-icons/fa";
import "../implementer/implementer.css";

const MODULES = ["Registration", "Clinical", "Lab", "Reports", "Admin"];

function AdminRoles() {
  const [roles, setRoles] = useState([
    { id: 1, name: "Doctor", permissions: { Registration: true, Clinical: true, Lab: true, Reports: true, Admin: false } },
    { id: 2, name: "Receptionist", permissions: { Registration: true, Clinical: false, Lab: false, Reports: false, Admin: false } },
  ]);
  ...
  return (
    <div className="content-area impl-page">
      ...
      <p className="section-subtitle">Create roles and toggle module access permissions</p>
                ...
                {MODULES.map((m) => (
                  <td key={m}>
                    <label className="impl-toggle">
                      <input type="checkbox" checked={r.permissions[m] || false} onChange={() => togglePermission(r.id, m)} />
                      {r.permissions[m] ? "On" : "Off"}
                    </label>
                  </td>
                ))}
              </tr>
