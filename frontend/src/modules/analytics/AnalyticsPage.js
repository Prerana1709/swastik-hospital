...
function AnalyticsPage() {
  ...
  useEffect(() => {
    api.getBillingStats().then(setBillingStats).catch(console.error);
    api.getDashboardCounts().then(setCounts).catch(console.error);
  }, []);

  const patientsToday = useMemo(() => counts.patients, [counts.patients]);
  const opdToday = useMemo(() => counts.opd, [counts.opd]);
  const revenueToday = useMemo(() => billingStats?.revenue_today || 0, [billingStats]);
  const suicideRiskCases = useMemo(() => Math.round(counts.patients * 0.05), [counts.patients]);
  const therapyCompletionRate = useMemo(() => "78%", []);

  const revenueTrends = useMemo(() => billingStats?.revenue_trends || REVENUE_6_MONTHS, [billingStats]);
  const summaryCards = useMemo(() => billingStats?.summary_cards || [], [billingStats]);
          ...
          {section === "financial" && (
            <div className="analytics-content">
              <div className="analytics-kpi-grid">
                {summaryCards.length > 0 ? (
                  summaryCards.map((card, i) => (
                    <KPIBox key={i} title={card.title} value={card.value} accent={card.accent} />
                  ))
                ) : (
                  <>
              ...
              <ChartsSection title="Revenue Trend" insight="Revenue up 8% MoM.">
                <ResponsiveContainer width="100%" height={280}>
                  <LineChart data={revenueTrends} margin={{ top: 5, right: 20, left: 0, bottom: 5 }}>
                    <CartesianGrid strokeDasharray="3 3" stroke="#eee" />
                    <XAxis dataKey="month" tick={{ fill: "#555", fontSize: 12 }} />
